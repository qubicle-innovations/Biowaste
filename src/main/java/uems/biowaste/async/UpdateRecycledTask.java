...
import uems.biowaste.RecycledCreateActivity;
...
public class UpdateRecycledTask extends
        AsyncTask<String, Void, TResponse<String>> {

	private Context ctx;
 	private TCustomProgressDailogue pd;
	public UpdateRecycledTask(Context context) {
		this.ctx = context;
	...
	protected void onPostExecute(TResponse<String> result) {
		...
		if (ctx instanceof RecycledCreateActivity) {
			...
			((HomeActivity) ctx).updateResponseRecycled(result);
		}
